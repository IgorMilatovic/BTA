...
namespace BTA.Controllers
{
    public class CommentsController : Controller
    {
        ...
        // GET: Comments
        public async Task<ActionResult> LoadComm(long? parentId)
        {
            var comments = db.Comments.Where(c => c.ParentId == parentId && c.TableName == "Comments").Select(c => c).OrderByDescending(c => c.Date).ToListAsync();
            return PartialView(await comments);
        }
        ...
        // GET: Comments/Create
        public ActionResult CreateShortComment()
        {
            return PartialView();
        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> CreateShortComment([Bind(Include = "CommentId,ParentId,Traveler,Title,Date,Text,Rating,Active,TableName")] Comment comment, long? parentId)
        {
            var identityUser = User.Identity.GetUserId();
            comment.Traveler = db.Travelers.Where(t => t.IdentityId == identityUser).Select(t => t.UserId).FirstOrDefault();
            comment.Date = DateTime.Now;
            ...
            comment.ParentId = parentId;
            var grandParent = db.Comments.Where(c => c.CommentId == parentId).Select(c => c.ParentId).FirstOrDefault();
            if (ModelState.IsValid)
            {
                db.Comments.Add(comment);
                await db.SaveChangesAsync();
            ...
            ViewBag.Traveler = new SelectList(db.Travelers, "UserId", "IdentityId", comment.Traveler);
            return RedirectToAction("Details", "POIs", new { id = grandParent });
        }
